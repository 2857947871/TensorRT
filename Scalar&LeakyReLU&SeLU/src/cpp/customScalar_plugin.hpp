...
// 定义命名空间(plugin在此命名空间下, 与python中的custom::customScalar一致)
namespace custom{

// PLUGIN_NAME(插件名称, 与python中的custom::customScalar一致)
...
public:
    ...
    */
    ...
    // 默认构造函数
    CustomScalarPlugin() = delete;  // 禁止默认构造函数

    // parse 时候用的构造函数
    CustomScalarPlugin(const std::string &name, float scalar, float sacle); 

    // clone&deseriaze 时候用的构造函数
    CustomScalarPlugin(const std::string &name, const void* buffer, size_t length);
